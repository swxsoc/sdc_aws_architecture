...
LANES = [
    dict(
        key="repo",
        title="Repository",
        sub="source code & infrastructure definitions",
        cat="repo",
        h=142,
    ),
    dict(
        key="build",
        title="Build & Image",
        sub="CodeBuild pipelines & ECR image registries",
        cat="build",
        h=150,
    ),
    dict(
        ...
        sub="file events, schedules & runtime secrets",
        cat="event",
        h=150,
    ),
    dict(
        key="run",
        title="Lambda Container Runtime",
        sub="containerized Lambda functions",
        cat="lambda",
        h=158,
    ),
    dict(
        key="data",
        title="Data & State",
        sub="science outputs, file state & time-series",
        cat="s3",
        h=142,
    ),
    dict(
        ...
        sub="mission packages, shared helpers & external services",
        cat="ext",
        h=164,
    ),
    dict(
...
def N(id, lane, cx, w, icon, title, sub="", lines=2):
    return dict(
        id=id, lane=lane, cx=cx, w=w, icon=icon, title=title, sub=sub, lines=lines
    )
...
NODES = [
    ...
    N(
        ...
        "repo",
        "Base image repos",
        "sdc_aws_base_docker_image (+ swxsoc_pipeline)",
    ),
    N(
        "r_pkgs",
        "repo",
        ...
        "repo",
        "Shared packages",
        "swxsoc · swxsoc_reach · sdc_aws_utils",
    ),
    N(
        "r_lam",
        "repo",
    ...
    N(
        ...
        "repo",
        "Deployment / IaC repo",
        "sdc_aws_architecture (Terraform)",
    ),
    # Build & image lane
    N(
        "b_cbbase",
    ...
    N(
        ...
        380,
        "s3",
        "Instrument / processed S3",
        "sorted & calibrated products",
    ),
...
EDGES = [
    ...
    E(
        ...
        "deployed images",
        via=[
            (_A("b_pvtecr", "b")[0], gap_below("build")),
            (GUT_R1, gap_below("build")),
            (GUT_R1, lane_top_pad("run")),
            (_A("l_alert", "t")[0], lane_top_pad("run")),
        ],
    ),
    ...
    E(
        ...
        via=[
            (GUT_R2, _A("r_dep", "r")[1]),
            (GUT_R2, lane_top_pad("run", 28)),
            (_A("l_alert", "tr")[0], lane_top_pad("run", 28)),
        ],
        lblpos=0.32,
    ),
    ...
    E(
        ...
        via=[
            (CH_SNS, _A("d_s3", "r")[1]),
            (CH_SNS, lane_top_pad("event")),
            (_A("e_sns", "t")[0], lane_top_pad("event")),
        ],
        lblpos=0.12,
    ),
    ...
    E(
        ...
        "",
        via=[
            (_A("l_proc", "bl")[0], gap_below("run")),
            (CH_INST, gap_below("run")),
            (CH_INST, lane_top_pad("ext")),
            (_A("x_inst", "t")[0], lane_top_pad("ext")),
        ],
    ),
    E(
        ("l_proc", "b"),
        ...
        "",
        via=[
            (_A("l_proc", "b")[0], gap_below("run")),
            (CH_UTILS, gap_below("run")),
            (CH_UTILS, lane_top_pad("ext")),
            (_A("x_utils", "t")[0], lane_top_pad("ext")),
        ],
    ),
    E(
        ("l_exec", "bl"),
        ...
        "import GOES flux",
        via=[
            (_A("l_exec", "bl")[0], gap_below("run")),
            (CH_GOES, gap_below("run")),
            (CH_GOES, lane_top_pad("ext")),
            (_A("x_goes", "t")[0], lane_top_pad("ext")),
        ],
    ),
    E(
        ("l_exec", "br"),
        ...
        "scheduled ingest",
        via=[
            (_A("l_exec", "br")[0], gap_below("run")),
            (CH_STIX, gap_below("run")),
            (CH_STIX, lane_top_pad("ext")),
            (_A("x_stix", "t")[0], lane_top_pad("ext")),
        ],
        lblpos=0.84,
    ),
    ...
    E(
        ...
        "uploads",
        via=[
            (FARL_UP, _A("x_reach", "l")[1]),
            (FARL_UP, lane_top_pad("event")),
            (_A("e_s3in", "t")[0], lane_top_pad("event")),
        ],
        lblpos=0.55,
    ),
    # ---- observability ----
    E(
        ...
        "Slack notifications",
        via=[
            (_A("l_sort", "b")[0], gap_below("run")),
            (FARL_DN, gap_below("run")),
            (FARL_DN, lane_top_pad("ops")),
            (_A("o_slack", "t")[0], lane_top_pad("ops")),
        ],
        lblpos=0.14,
    ),
    E(
        ...
        "dashboards",
        via=[
            (_A("d_ts", "b")[0], gap_below("data")),
            (CH_GRAF, gap_below("data")),
            (CH_GRAF, lane_top_pad("ops")),
            (_A("o_graf", "t")[0], lane_top_pad("ops")),
        ],
        lblpos=0.5,
    ),
...
def rounded_path(pts, r=16):
    ...
    for i in range(1, len(pts) - 1):
        ...
        d += " L{} {} Q{} {} {} {}".format(
            round(a[0], 1), round(a[1], 1), p1[0], p1[1], round(b[0], 1), round(b[1], 1)
        )
...
def glyph(kind):
    ...
    if kind == "repo":
        return (
            '<path d="M14 11h15a3 3 0 0 1 3 3v23H17a3 3 0 0 1-3-3V11z" fill="none" stroke="%s" stroke-width="2.4"/>'
            '<path d="M19 18h11M19 24h11M19 30h7" stroke="%s" stroke-width="2.6" stroke-linecap="round"/>'
            % (w, w)
        )
    if kind == "build":
        return (
            '<circle cx="24" cy="24" r="8.4" fill="none" stroke="%s" stroke-width="2.6"/>'
            '<path d="M24 9v5M24 34v5M9 24h5M34 24h5M13.5 13.5l3.5 3.5M31 31l3.5 3.5M34.5 13.5L31 17M17 31l-3.5 3.5" stroke="%s" stroke-width="2.4" stroke-linecap="round"/>'
            % (w, w)
        )
    if kind == "registry":
        return (
            '<rect x="11" y="11" width="26" height="8" rx="1.6" fill="%s"/>'
            '<rect x="11" y="21.5" width="26" height="8" rx="1.6" fill="%s" opacity="0.85"/>'
            '<rect x="11" y="32" width="17" height="7" rx="1.6" fill="%s" opacity="0.7"/>'
            % (w, w, w)
        )
    if kind == "s3":
        return (
            '<path d="M12 14h24l-2.4 22a2 2 0 0 1-2 1.8H16.4a2 2 0 0 1-2-1.8L12 14z" fill="none" stroke="%s" stroke-width="2.5" stroke-linejoin="round"/>'
            '<path d="M12.7 20h22.6" stroke="%s" stroke-width="2.4"/>' % (w, w)
        )
    if kind == "db":
        return (
            '<ellipse cx="24" cy="13.5" rx="12" ry="4.2" fill="none" stroke="%s" stroke-width="2.5"/>'
            '<path d="M12 13.5v21c0 2.3 5.4 4.2 12 4.2s12-1.9 12-4.2v-21" fill="none" stroke="%s" stroke-width="2.5"/>'
            '<path d="M12 24c0 2.3 5.4 4.2 12 4.2s12-1.9 12-4.2" fill="none" stroke="%s" stroke-width="2.2"/>'
            % (w, w, w)
        )
    if kind == "event":
        return (
            '<circle cx="15" cy="24" r="3.4" fill="%s"/>'
            '<circle cx="34" cy="14" r="3.4" fill="%s"/>'
            '<circle cx="34" cy="24" r="3.4" fill="%s"/>'
            '<circle cx="34" cy="34" r="3.4" fill="%s"/>'
            '<path d="M18 24h6M30.6 15.4 24 24M30.6 32.6 24 24M27 24h3.6" stroke="%s" stroke-width="2.2" stroke-linecap="round"/>'
            % (w, w, w, w, w)
        )
    if kind == "secret":
        return (
            '<circle cx="20" cy="19" r="6.6" fill="none" stroke="%s" stroke-width="2.6"/>'
            '<path d="M24.4 23.4 36 35M31 30l3.4-3.4M34 33l3-3" stroke="%s" stroke-width="2.6" stroke-linecap="round"/>'
            % (w, w)
        )
    if kind == "lambda":
        return (
            '<path d="M16 11h7l13 26h-7.4l-9.6-19.6L24 28l-4.6 9H12L16 11z" fill="%s"/>'
            % w
        )
    if kind == "ext":
        return (
            '<circle cx="24" cy="24" r="13" fill="none" stroke="%s" stroke-width="2.5"/>'
            '<path d="M11 24h26M24 11v26" stroke="%s" stroke-width="2.1"/>'
            '<path d="M24 11c5 4.5 5 21.5 0 26M24 11c-5 4.5-5 21.5 0 26" fill="none" stroke="%s" stroke-width="2.1"/>'
            % (w, w, w)
        )
    if kind == "slack":
        return (
            '<path d="M19 14v20M29 14v20M14 19h20M14 29h20" stroke="%s" stroke-width="3.1" stroke-linecap="round"/>'
            % w
        )
    if kind == "grafana":
        return (
            '<path d="M12 35 21 24l6 5 9-13" fill="none" stroke="%s" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>'
            '<circle cx="21" cy="24" r="2.6" fill="%s"/><circle cx="27" cy="29" r="2.6" fill="%s"/>'
            % (w, w, w)
        )
    if kind == "log":
        return (
            '<path d="M13 13h22M13 20h16M13 27h22M13 34h13" stroke="%s" stroke-width="2.7" stroke-linecap="round"/>'
            % w
        )
    if kind == "ec2":
        return (
            '<rect x="15" y="15" width="18" height="18" rx="2.5" fill="none" stroke="%s" stroke-width="2.4"/>'
            '<rect x="20" y="20" width="8" height="8" rx="1.2" fill="%s"/>'
            '<path d="M19 15v-4M24 15v-4M29 15v-4M19 33v4M24 33v4M29 33v4M15 19h-4M15 24h-4M15 29h-4M33 19h4M33 24h4M33 29h4" stroke="%s" stroke-width="2" stroke-linecap="round"/>'
            % (w, w, w)
        )
    return ""
...
add(
    ...
    'viewBox="0 0 {W} {H}" font-family="{F}" role="img" aria-labelledby="ttl dsc">'.format(
        W=W, H=H, F=FONT
    )
)
add('<title id="ttl">SWxSOC AWS Architecture</title>')
add(
    '<desc id="dsc">Lane-based AWS architecture diagram for SWxSOC: repositories build Lambda '
...
add(
    ...
    '<stop offset="0" stop-color="{a}"/><stop offset="1" stop-color="{b}"/></linearGradient>'.format(
        a=PAPER_A, b=PAPER_B
    )
)
for k, v in CAT.items():
    g0, g1 = v["g"]
    add(
        '<linearGradient id="ic-{k}" x1="0" y1="0" x2="0" y2="1">'
        '<stop offset="0" stop-color="{a}"/><stop offset="1" stop-color="{b}"/></linearGradient>'.format(
            k=k, a=g0, b=g1
        )
    )
...
add(
    '<filter id="cloudshadow" x="-5%" y="-5%" width="110%" height="115%">'
...
for name, col in FLOW.items():
    add(
        '<marker id="ar-{n}" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7.5" markerHeight="7.5" orient="auto-start-reverse">'
...
# ---- title block ----
add(
    '<rect x="{x}" y="56" width="64" height="40" rx="7" fill="{ink}"/>'.format(
        x=MARGIN + 8, ink=INK
    )
)
add(
    '<text x="{x}" y="83" font-size="20" font-weight="700" fill="#ff9d2e" text-anchor="middle">aws</text>'.format(
        x=MARGIN + 8 + 32
    )
)
add(
    '<text x="{x}" y="90" font-size="46" font-weight="800" fill="{tx}" letter-spacing="0.3">SWxSOC AWS Architecture</text>'.format(
        x=MARGIN + 88, tx=INK
    )
)
add(
    '<rect x="{x}" y="104" width="232" height="6" rx="3" fill="url(#accent)"/>'.format(
        x=MARGIN + 90
    )
)
add(
    '<text x="{x}" y="136" font-size="19" fill="{sub}">Container build pipeline and runtime data flow: source repos build Lambda images, '
    "AWS events invoke containers, and science outputs feed storage, state, alerts and dashboards.</text>".format(
        x=MARGIN + 90, sub=TX_SUB
    )
)

# ---- AWS Cloud boundary ----
add(
    '<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="22" fill="#ffffff" stroke="{ink}" stroke-width="2.4" filter="url(#cloudshadow)"/>'.format(
        x=MARGIN + 6,
        y=CLOUD_TOP,
        w=W - 2 * (MARGIN + 6),
        h=CLOUD_BOT - CLOUD_TOP,
        ink=INK,
    )
)
add(
    '<rect x="{x}" y="{y}" width="148" height="34" rx="7" fill="{ink}"/>'.format(
        x=MARGIN + 28, y=CLOUD_TOP + 18, ink=INK
    )
)
add(
    '<text x="{x}" y="{y}" font-size="17" font-weight="700" fill="#ffffff">AWS Cloud</text>'.format(
        x=MARGIN + 44, y=CLOUD_TOP + 41
    )
)
# region boundary
add(
    '<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="16" fill="none" stroke="#2296b7" stroke-width="2" stroke-dasharray="11 8"/>'.format(
        x=LANE_L - 22,
        y=REGION_TOP,
        w=(LANE_R + 22) - (LANE_L - 22),
        h=REGION_BOT - REGION_TOP,
    )
)
add(
    '<rect x="{x}" y="{y}" width="34" height="34" rx="7" fill="#2296b7"/>'.format(
        x=LANE_L - 8, y=REGION_TOP + 14
    )
)
add(
    '<text x="{x}" y="{y}" font-size="16" font-weight="700" fill="#14748d">Region</text>'.format(
        x=LANE_L + 34, y=REGION_TOP + 30
    )
)
add(
    '<text x="{x}" y="{y}" font-size="14" fill="#5a7a86">SWxSOC runtime account &amp; delivery path</text>'.format(
        x=LANE_L + 108, y=REGION_TOP + 30
    )
)
...
for L in LANES:
    ...
    a = CAT[L["cat"]]["a"]
    add(
        '<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="14" fill="{f}" stroke="{s}" stroke-width="1.5"/>'.format(
            x=LANE_L, y=y, w=LANE_R - LANE_L, h=L["h"], f=tint(a, 0.90), s=tint(a, 0.45)
        )
    )
    add(
        '<rect x="{x}" y="{y}" width="7" height="{h}" rx="3.5" fill="{a}"/>'.format(
            x=LANE_L + 0, y=y, h=L["h"], a=a
        )
    )
...
def draw_lane_titles():
    for L in LANES:
        ...
        add(
            '<rect x="{x}" y="{cy}" width="{w}" height="30" rx="8" fill="{f}"/>'.format(
                x=chip_x, cy=y + 11, w=chip_w, f=tint(a, 0.90)
            )
        )
        add(
            '<text x="{x}" y="{ty}" font-size="20" font-weight="700" fill="{tx}">{t}</text>'.format(
                x=LANE_L + 26, ty=y + 32, tx=INK, t=esc(L["title"])
            )
        )
        add(
            '<text x="{x}" y="{ty}" font-size="13.5" fill="{sub}">{s}</text>'.format(
...
for e in EDGES:
    ...
    if e["emph"]:
        ...
        add(
            '<path d="{d}" fill="none" stroke="{c}" stroke-width="9" opacity="0.16" '
            'stroke-linejoin="round" stroke-linecap="round"/>'.format(d=d, c=col)
        )
        add(
            '<path d="{d}" fill="none" stroke="{c}" stroke-width="3.8"{dash} '
        ...
        add(
            '<path d="{d}" fill="none" stroke="{c}" stroke-width="2.4"{dash} '
            'marker-end="url(#ar-{f})" stroke-linejoin="round" stroke-linecap="round"/>'.format(
                d=d, c=col, dash=dash, f=e["flow"]
            )
        )
...
for lx, ly, text, col, emph in label_boxes:
    if emph:
        ...
        add(
            ...
            "</g>".format(
                x=lx - wpx / 2, y=ly - 11, w=wpx, c=col, tx=lx, ty=ly + 4.2, t=esc(text)
            )
        )
...
def draw_node(n):
    ...
    add(
        '<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="13" fill="{card}" stroke="{brd}" stroke-width="1.4"/>'.format(
            x=x, y=y, w=w, h=h, card=CARD, brd=CARD_BRD
        )
    )
    add(
        '<rect x="{x}" y="{y}" width="6" height="{h}" rx="3" fill="{a}"/>'.format(
            x=x, y=y, h=h, a=a
        )
    )
    add("</g>")
    ...
    sc = isz / 48.0
    add(
        '<g transform="translate({ix},{iy}) scale({sc})">'.format(
            ix=round(ix, 1), iy=round(iy, 1), sc=round(sc, 4)
        )
    )
    add(
        '<rect width="48" height="48" rx="11" fill="url(#ic-{k})"/>'.format(k=n["icon"])
    )
    add(
        '<rect width="48" height="48" rx="11" fill="none" stroke="#ffffff" stroke-opacity="0.28" stroke-width="1"/>'
    )
    add(glyph(n["icon"]))
    add("</g>")
    tx = ix + isz + 16
    if n["lines"] == 2:
        add(
            '<text x="{tx}" y="{ty}" font-size="16.5" font-weight="700" fill="{c}">{t}</text>'.format(
                tx=tx, ty=y + h / 2 - 5, c=TX, t=esc(n["title"])
            )
        )
        add(
            '<text x="{tx}" y="{ty}" font-size="13" fill="{c}">{s}</text>'.format(
...
lx = LANE_L + 4
add(
    '<text x="{x}" y="{y}" font-size="14.5" font-weight="700" fill="{ink}">Legend</text>'.format(
        x=lx, y=ly + 16, ink=INK
    )
)
lx += 96
for flow, text in legend:
    ...
    add(
        '<rect x="{x}" y="{y}" width="26" height="14" rx="4" fill="{c}"/>'.format(
            x=lx, y=ly + 3, c=col
        )
    )
    add(
        '<text x="{x}" y="{y}" font-size="14" fill="{tx}">{t}</text>'.format(
            x=lx + 34, y=ly + 15, tx=TX, t=esc(text)
        )
    )
    lx += 56 + 8.2 * len(text)
...
def draw_fleet():
    ...
    add(
        '<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="20" fill="#f3f6fb" stroke="#9fb0c3" stroke-width="1.8"/>'.format(
            x=px, y=FLEET_TOP, w=pw, h=FLEET_H
        )
    )
    add(
        '<rect x="{x}" y="{y}" width="334" height="34" rx="8" fill="{ink}"/>'.format(
            x=px + 26, y=FLEET_TOP + 20, ink=INK
        )
    )
    add(
        '<text x="{x}" y="{y}" font-size="17" font-weight="700" fill="#ffffff">Deployment Fleet &amp; Inventory</text>'.format(
            x=px + 42, y=FLEET_TOP + 43
        )
    )
    add(
        '<text x="{x}" y="{y}" font-size="14" fill="{sub}">the logical pipeline above is realized by this fleet — account smdc-aws-swsoc · us-east-1</text>'.format(
    ...
    def card(i, icon, title, lines):
        ...
        ix, iy = cx + 22, cy + 18
        add(
            '<g transform="translate({ix},{iy}) scale({sc})">'
        ...
        add(
            '<text x="{x}" y="{y}" font-size="18" font-weight="700" fill="{tx}">{t}</text>'.format(
        ...
        for txt, style in lines:
            if style == "b":
                add(
                    '<text x="{x}" y="{y}" font-size="14.5" font-weight="700" fill="{c}">{t}</text>'.format(
                        x=cx + 24, y=yy, c=TX, t=esc(txt)
                    )
                )
                yy += 25
            elif style == "m":
                add(
                    '<text x="{x}" y="{y}" font-size="13" fill="{c}">{t}</text>'.format(
                        x=cx + 40, y=yy, c=TX_SUB, t=esc(txt)
                    )
                )
                yy += 23
            else:
                add(
                    '<text x="{x}" y="{y}" font-size="13.5" fill="{c}">{t}</text>'.format(
                        x=cx + 24, y=yy, c="#34414f", t=esc(txt)
                    )
                )
                yy += 25
