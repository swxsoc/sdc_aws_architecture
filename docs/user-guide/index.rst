...
* Python 3.12.x (only needed for building docs or running helper scripts)
...
.. code-block:: bash
    ...
    git clone https://github.com/swxsoc/sdc_aws_architecture.git
    cd sdc_aws_architecture
...
Use `dev-<mission>` for development and `prod-<mission>` for production. The workspace prefix controls resource naming. Create the workspace only when bootstrapping a new mission/environment.
